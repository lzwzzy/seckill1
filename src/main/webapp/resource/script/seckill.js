...
var seckill = {
    ...
    URL : {
            ...
            exxcution: function (seckillId,md5) {
                return '/seckill/'+seckillId+'/'+md5+'/execution';
            }
    },
    //处理秒杀逻辑
    handleSeckillkill: function (seckillId,seckillBox) {
        seckillBox.hide().html('<button class="btn btn-primary btn-lg" id="killBtn">开始秒杀</button>');//秒杀按钮
        $.post(seckill.URL.exposer(seckillId),{},function (result) {
            ...
            if (result && result['success']){
                ...
                if (exposer['exposed']){
                    //开启秒杀
                    var md5 = exposer['md5'];
                    var killUrl = seckill.URL.exxcution(seckillId,md5);
